...
public class RupayUnmatchTTUMServiceImpl extends JdbcDaoSupport implements RupayUnmatchTTUMService {
	...
	public HashMap<String,Object> checkTTUMProcessed(UnMatchedTTUMBean beanObj)
	{
		...
		try
		{
			...
			{
				query = query+" ttum_rupay_sur_cbs where filedate = str_to_date('"+beanObj.getLocalDate()+"','%Y/%m/%d') "
			...
			{
				query = query+" ttum_rupay_dom_cbs where tran_date = str_to_date('"+beanObj.getLocalDate()+"','%Y/%m/%d') ";
						
			}
			else if(beanObj.getTypeOfTTUM().equals("UNRECON"))
			{
				query = query+" ttum_rupay_dom_switch where tran_date = str_to_date('"+beanObj.getLocalDate()+"','%Y/%m/%d') ";
	...
	public HashMap<String,Object> checkReconDateAndTTUMDataPresent(UnMatchedTTUMBean beanObj)
	{
		...
		try
		{
			...
			query = query + "settlement_rupay_dom_cbs where filedate >= str_to_date('"+beanObj.getLocalDate()+"','%Y/%m/%d') ";
			...
			if(recordCount > 0)
			{
				...
				{
					query = "select count(1) from settlement_rupay_dom_cbs where filedate = "
							+ "(select max(filedate) from settlement_rupay_dom_cbs) "
							+" AND dcrs_remarks LIKE '%RUPAY_DOM-UNRECON-1 (%' and respcode = '00' "
				...
				{
					query = "select count(1) from settlement_rupay_dom_switch where filedate = "
							+ "(select max(filedate) from settlement_rupay_dom_cbs) "
							+" AND dcrs_remarks = 'RUPAY_DOM-UNRECON-2' and respcode = '00' "
...
public Boolean RupayTtumRollback(UnMatchedTTUMBean beanObj)
{
	...
	try
	{
		if(beanObj.getTypeOfTTUM().equals("SURCHARGE"))
		{
			deleteQuery = "delete from ttum_rupay_sur_cbs where filedate = str_to_date('"
			...
			updateQuery = "update settlement_rupay_dom_cbs set dcrs_remarks = 'RUPAY_SUR-MATCHED-3' where "
					...
					+" and filedate = str_to_date('"+beanObj.getLocalDate()
					+"','%Y/%m/%d') "
					+"and (mcc not in ('5541','5542','5983') "
					+" or (mcc in ('5541','5542','5983') and substring(remarks,1,6) = '436359'))";
		...
		{
			deleteQuery = "delete from ttum_rupay_dom_cbs where tran_date = str_to_date('"
					+beanObj.getLocalDate()+"','%Y/%m/%d') ";
			getJdbcTemplate().execute(deleteQuery);
			...
			updateQuery = "update settlement_rupay_dom_cbs set dcrs_remarks = 'RUPAY_DOM-UNRECON-1 (' where "
					...
					+"','%Y/%m/%d')-- and filedate = (select max(filedate) from settlement_rupay_dom_cbs) ";
		...
		{
			deleteQuery = "delete from ttum_rupay_dom_switch where tran_date = str_to_date('"
					+beanObj.getLocalDate()+"','%Y/%m/%d') ";
			getJdbcTemplate().execute(deleteQuery);
...
public List<String> getRupayTTUMDataForTXT(UnMatchedTTUMBean beanObj) {
	...
	try
	{
		...
		if(beanObj.getTypeOfTTUM().equalsIgnoreCase("SURCHARGE") && beanObj.getTypeOfTTUM().equalsIgnoreCase("WAIVER"))
		{
		...
		else
		{
			 getData = "select concat('1',date_format(sysdate(),'%Y%m%d')) as a from dual "
						...
						+" UNION ALL "		 
						+" select  concat('3',(select lpad(count(1),9,'0') "
						+" from "+ttum_tableName+" WHERE tran_date = str_to_date('"+beanObj.getLocalDate()+"','%Y/%m/%d') "
