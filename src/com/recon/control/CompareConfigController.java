...
public class CompareConfigController {
	...
	@Autowired ICompareConfigDao compareconfigDao;
	...
	HashMap<String, Object> output = new HashMap<String, Object>() ;
		...
		phyFileName =file.getOriginalFilename();
			...
			try {
				//if( !(icompareConfigService.chkBeforeUploadFile(chkFlag, setupBean ))){ //commented by int8624 on 19/jan/2022
				//ADDED on 28 mar
				...
				if(!checkFileName)
				{
					...
					if(!setupBean.getFilename().equalsIgnoreCase("REV_REPORT"))
					{
						output = icompareConfigService.checkUploadedFileName(setupBean.getCategory(), setupBean.getStSubCategory(), phyFileName);
					...
					if(output!= null && (Boolean)output.get("result"))
					{
						if( icompareConfigService.chkUploadFlag(chkFlag, setupBean).equalsIgnoreCase("N") && (!setupBean.getFilename().equals("REV_REPORT"))){
							if(!file.isEmpty()) {
								...
								stream.close();*/
									...
									output = icompareConfigService.uploadFile(setupBean,file);
							...
							if(output != null && (Boolean) output.get("result")){
								...
								return new ResponseEntity("File Uploaded Successfuly!! \n Total Record Count : "+count+"", HttpStatus.OK); 
