	...
	public String chkFileUpload(String Category, String filedate,
		...
		try {
			for (CompareSetupBean setupBean : compareSetupBeans) {
				...
				{
					getMainFileCount = "select count(*) from main_file_upload_dtls where filedate = str_to_date('"+filedate+
							"','%Y/%m/%d') and fileid = '"+setupBean.getInFileId()+"' and file_count = '"+fileCount+"'";
...
public String validateFile(String category,
	...
	try
	{
		...
		String filedt = filedate.replace("/", "-"); 
		...
		if(error_msg == null)
		{
			...
			else
			{
				for (CompareSetupBean setupBean : compareSetupBeans) {
					...
					String query = "select count(*) from main_file_upload_dtls where substr(filedate,1,10)  = str_to_Date('"+filedt+"','%Y-%m-%d')- interval '1' day "
		...
		try {
			if(category.equalsIgnoreCase("RUPAY"))
			{
				if(subcat.equalsIgnoreCase("DOMESTIC"))
				{
					...
					String getCount = "select count(*) from main_file_upload_dtls a where fileid in ( select fileid from main_filesource where file_category = 'RUPAY' and file_subcategory = 'DOMESTIC') "+        
							" AND filedate = str_to_date(?,'%Y/%m/%d') and filter_flag != (select filteration FROM main_filesource b where a.fileid = b.fileid)";
			...
			else if (category.equalsIgnoreCase("VISA"))
			{
				...
				if(subcat.equalsIgnoreCase("ISSUER"))
				{
					...
					String getCount = "select count(*) from main_file_upload_dtls a where fileid in ( select fileid from main_filesource where file_category = 'VISA' and file_subcategory = 'ISSUER') "+        
							" AND filedate = str_to_date(?,'%Y/%m/%d') and filter_flag != (select filteration FROM main_filesource b where a.fileid = b.fileid)";
		...
		try {
			...
			if (FilterFlag1.equalsIgnoreCase("Y")
				...
				sql = "Select count(*) from MAIN_FILE_UPLOAD_DTLS where  FILEID = "
						+ fileid1
						+ " and Knockoff_FLAG='Y' AND Upload_FLAG = 'Y' and FILTER_FLAG = 'Y' and COMAPRE_FLAG='N' and  filedate =  str_to_date('"
				...
				sql = "Select count(*) from MAIN_FILE_UPLOAD_DTLS where FILEID = "
						+ fileid1
						+ " and  Upload_FLAG = 'Y' and  COMAPRE_FLAG='N' and filedate =  str_to_date('"
